...
Diese Vorlage hat das Ziel, Studierende in den technischen Studiengängen, insbesondere den Ingenieurwissenschaften, dabei zu helfen, die Defizite der allgemeinen Vorlage der Staatlichen Studienakademie Dresden\footnote{\label{fn:Leitfaden}Im Genauen: \enquote{Verbindlicher Leitfaden für die Anfertigung und formale Gestaltung wissenschaftlicher (Haus-)Arbeiten an der der Staatlichen Studienakademie Dresden}} für diesen Fachbereich auszubessern.
...
\begin{itemize}
    \item{
        ...
        \begin{itemize}
            \item Doppelseitiges Layout (wenn Sie explizit einseitiges Layout wünschen, müssen Sie in der Datei \textit{thesis.tex} in Zeile $30$ \enquote{twoside} auskommentieren),
    ...
    \item{
        ...
        \begin{itemize}
            ...
            \item Standarde (DIN, ISO, RFC, ...) oder
...
\begin{itemize}
    \item \texttt{\textbackslash setlength\{\textbackslash parindent\}\{0pt\}} --\,definiert den Zeilenvorschub, hier auf Null, und
    \item \texttt{\textbackslash setlength\{\textbackslash parskip\}\{4pt\}} --\,definiert die Absatzdistanz, hier auf 4 Punkte.
\end{itemize}

Die Anweisung \texttt{\textbackslash noindent} zu Beginn eines Absatzes unterdrückt den Zeilenvorschub. Das ist von Relevanz, falls Sie einen Wert ungleich Null für das \texttt{\textbackslash parindent} definiert haben. Es ist nämlich Standardeinstellung von \LaTeX{}, dass der erste Absatz nach der Überschrift des Kapitels bzw. der Sektion nicht eingerückt wird, alle anderen aber schon. Das ist unschön, wenn Sie ein Blockelement im Fließtext haben, wie zum Beispiel zu Beginn dieses Kapitels nach dem Kommentar. Wie Sie im Quelltext sehen können, ist an der entsprechenden Stelle ein \texttt{\textbackslash noindent} platziert. Beobachten Sie das Verhalten, wenn Sie in der \texttt{config.tex} den Wert für \texttt{\textbackslash parindent} verändern!

Apropos Absätze: {\color{red}Absatzumbrüche erzeugen Sie in \LaTeX\ immer mit einem doppelten Zeilenumbruch im Quelltext.} Achten Sie also drauf, wo Sie einzelne und doppelte Zeilenumbrüche im Quelltext verwenden.

Gerne verwenden Studierende für wichtige Begriffe einen \Gls{glossar}. Das ist recht einfach und wird in dieser Vorlage mit dem \enquote{glossaries}-Paket umgesetzt. Alle Glossareinträge befinden sich in der Datei \texttt{frontbackmatter\textbackslash Glossary.tex}. Sie werden mit der Anweisung \texttt{\textbackslash Gls\{Eintrag\}} referenziert.
...
\begin{itemize}
    \item \texttt{\textbackslash acrshort\{abk\}}: erzeugt nur die Abkürzung, hier also \enquote{\acrshort{abk}},
    \item \texttt{\textbackslash acrlong\{abk\}}: erzeugt das Ausgeschriebene, hier also \enquote{\acrlong{abk}}, sowie
    \item \texttt{\textbackslash acrfull\{abk\}}: erzeugt beides, hier also \enquote{\acrfull{abk}}.
\end{itemize}

Außerdem gibt es, wie Sie sicherlich gemerkt haben, auch ein paar Tricks für die korrekte Referenzierung des Plurals. Dazu verwenden Sie die Anweisung \texttt{\textbackslash Glspl\{abk\}}. Beachten Sie, dass beim erstmaligen Aufruf (siehe oben) das Verhalten von \texttt{acrfull} verwendet wird.
...
Ein letzter Stilhinweis: Vermeiden Sie nach Möglichkeit Silber-, Wort-, Satz- und Absatzwaisen. D.h., dass Sie vermeiden eine neue Zeile oder gar eine ansonsten leere Seite mit nur einer Silbe, einem einzelnen Wort zu beginnen. Versuchen Sie, Seiten mit weniger als $5$ Zeilen zu vermeiden.
...
\begin{itemize}
    \item{Zwingen Sie das ganze Wort in die neue Zeile indem Sie eine \texttt{\textbackslash mbox\{\}} um das Wort legen. Das ist auch sinnvoll, wenn Sie Gesetze o.\,ä. referenzieren und sie zusammenbehalten wollen: \texttt{\textbackslash mbox\{§§ 126, 126a BGB\}}.}
    \item{Sie können \LaTeX{} explizit die Silbenstrennung vorgeben, indem Sie die Trennstellen mit \texttt{\textbackslash-} markieren: \texttt{Do\textbackslash-nau\textbackslash-dampf\textbackslash-schiff\textbackslash-fahrt}. Bei häufig verwendeten Wörtern definieren Sie die Silbentrennung lieber am Ende des Vorspanns mit der \texttt{\textbackslash hyphenation\{\}}-Umgebung (in dieser Vorlage in der \texttt{thesis.tex} in Zeile $47$). Aber Vorsicht: \LaTeX{} wird die Wörter dann \emph{nur} an diesen Stellen trennen.}
    \item{Nicht zu trennende Wörter können Sie anstatt mit einer \texttt{\textbackslash mbox\{\}} auch mit einer Tilde binden: \texttt{Versuch\textasciitilde 1} oder \texttt{§§\textasciitilde 126,\textasciitilde 126a\textasciitilde BGB}.}
\end{itemize}
